...
**Single-item binder — skip deliver.** When the binder has exactly one work item, hand straight to `karta-build`. There is no wave to schedule and no integration branch to assemble across multiple items. This is the "just this once" hatch: fast, unceremonious, correct. The end-of-life rule still applies, and in this mode `karta-build` owns it: its 9c-single sequence archives the binder per `deliver:archive` after its clean merge — there is no orchestrator left to do it.
