...
- **Fully automatic correction.** When on, the gardner **rewrites** drifted docs to match the current code and commits the fix. It does not just report.
...
- **Scope is never static.** What to gardner is recomputed every run from the live repo — never a stored doc list or a cached analysis — so files created after opt-in are always in scope.
...
- The gardner is a **writer**, not a reporter. It corrects drift directly. (keel: read-only + orchestrator applies.)
...
### 3a. Opt-in switch — `.karta/doc-gardner.json`
...
```json
{ "enabled": true, "focus": "optional freeform note biasing the gardner, e.g. 'keep the public API docs and the architecture overview honest'" }
```

- **Present and `enabled: true`** → opted in. Every `karta-deliver` run executes the doc-gardner phase; it cannot be skipped.
...
A JSON Schema (`references/doc-gardner-schema.json`) gates the file shape, and an example ships.

### 3b. Scope — recomputed every run, never stored

On each run the gardner re-derives its scope from the live repo, so nothing goes stale:
...
### 3c. The `karta-doc-gardner` agent (a writer)

A new agent, canonical at `agents/karta-doc-gardner.md`. Unlike the two read-only gate agents, it **edits docs**. Given the binder, the integration diff range, the live doc surface, and the optional `focus`, it:
...
- **`karta-doc-gardner` skill** — the sole spawn site for the agent (mirrors how `karta-verify` is the sole spawn site for the gate agents). It resolves the agent adaptively per runtime (registered subagent on Claude / Codex-with-TOML; bundled instructions on a Codex plugin install) and bundles the agent instructions at `references/karta-doc-gardner.agent.md` so it works on a Codex plugin install with no setup.
- **`karta-deliver` terminal phase `deliver:docgardner`** — after the final wave merges into the integration branch, if `.karta/doc-gardner.json` is opted in, karta-deliver invokes the `karta-doc-gardner` skill over the whole delivery's blast radius, then commits the corrections as a distinct `docs: gardner <slug>` commit on the integration branch. If opted out, the phase no-ops with a one-line note.
- **Single-item hatch** — `karta-build` invoked directly applies the same terminal step when opted in.
- **Standalone / ad-hoc** — a user can invoke `karta-doc-gardner` directly for a full-repo correction pass, independent of a delivery.

The correction lands as its own labeled commit so a human reviewing the integration branch (karta's normal review-and-merge point) sees exactly what the gardner changed and can revert it like any commit. That is the review surface — there is no inline waive.

### 3e. No human, no halt

The doc-gardner phase never blocks the delivery and never asks a person anything. It corrects, commits, and the delivery proceeds. The only record is the `docs: gardner` commit and the gardner's envelope folded into the delivery report.
...
- Canonical `agents/karta-doc-gardner.md` → generated `.codex/agents/karta-doc-gardner.toml` and the bundled `skills/karta-doc-gardner/references/karta-doc-gardner.agent.md`, both via `sync_codex_agents.py`.
- The new `skills/karta-doc-gardner/` is mirrored into `.agents/skills/` by `sync_codex_skills.py` and carries its own `agents/openai.yaml`.
- **Sandbox derivation.** `sync_codex_agents.py` sets `sandbox_mode` from the agent's declared tools: an agent whose `tools` include `Write`/`Edit` (the gardner) → `sandbox_mode = "workspace-write"`; otherwise → `read-only` (the two gates). On a Codex plugin-install fallback, `karta-doc-gardner` spawns a normal workspace-write worker (not the read-only `explorer`) with the bundled instructions.

## 5. Validator changes — `scripts/validate_plugin.py`

- The `.codex/agents/*.toml` sandbox check becomes per-agent: assert each agent's `sandbox_mode` equals the value derived from its canonical `tools` (read-only for the gates, workspace-write for the gardner) — not a blanket "must be read-only."
...
- New: if `.karta/doc-gardner.json` is committed as the repo's own example/config, validate it against `references/doc-gardner-schema.json`. (The schema + example live under the `karta-doc-gardner` skill.)

## 6. Docs

- README: a short "Automatic doc-gardner (opt-in)" subsection — how to turn it on, what it does, where the corrections land.
- `docs/how-to/doc-gardner.md`: enabling it, the `.karta/doc-gardner.json` shape, the dynamic-scope guarantee, what gets corrected, and how to review/revert the `docs: gardner` commit.
- AGENTS.md: note the new canonical agent + skill and that the gardner is the one *writer* agent.

## 7. Non-goals

No human/waive/halt/accept path. No severity tiers or advisory `INFO`. No keel-style P5 timeline doctrine. No static or cached doc list. No commit hook (the karta-deliver phase is the enforcement). The gardner does not refactor or expand docs beyond correcting drift, and it never edits code, tests, or the binder.

## 8. Risk note (accepted)

An LLM rewriting committed documentation automatically can mis-correct. This is accepted by explicit user direction (fully automatic, all-or-nothing). Mitigations that do not reintroduce a human gate: corrections are scoped strictly to detected drift; they land as a single labeled `docs: gardner` commit on the integration branch (never pushed, never on `main`), which is karta's normal human review-and-merge surface; and the gardner re-verifies its own output before committing.

## 9. File inventory

Created:

- `agents/karta-doc-gardner.md` (canonical, writer agent)
- `.codex/agents/karta-doc-gardner.toml` (generated, `workspace-write`)
- `skills/karta-doc-gardner/SKILL.md` + `skills/karta-doc-gardner/agents/openai.yaml`
- `skills/karta-doc-gardner/references/karta-doc-gardner.agent.md` (generated, bundled instructions)
- `skills/karta-doc-gardner/references/doc-gardner-schema.json` + an example `.karta/doc-gardner.json`
- `.agents/skills/karta-doc-gardner/…` (generated mirror)
- `docs/how-to/doc-gardner.md`

Modified:

- `skills/karta-deliver/SKILL.md` (new terminal phase `deliver:docgardner`, opt-in gated)
...
- `scripts/validate_plugin.py` (per-agent sandbox expectation; optional `.karta/doc-gardner.json` schema check)
- `README.md`, `AGENTS.md`
